...
## 積込・運搬数量機能（新規追加・加工数量とは完全独立）

### 概要
既存の「加工数量」機能とは業務上まったく別の機能として、「積込・運搬数量」を追加しました。加工数量の入力・一覧・分析・CSV・PDF・API・DB には一切変更を加えていません。運搬分析でも加工データは参照しません。

### 追加した画面（ナビゲーション末尾に3項目追加）
- **積込・運搬数量入力**：日付・工場・積込・運搬人員/人工（複数行）・積込・運搬車両（自由入力）・積込・運搬数量(kg)
- **積込・運搬実績一覧**：期間/工場/人員/車両（部分一致）で絞り込み、合計・件数・1人工当たり積込・運搬数量を表示、編集/削除、CSV/PDF出力
- **積込・運搬数量分析**：日別／月別／年別／人員別の4タブ、工場切替（全工場合算・本社工場・第二工場）、Chart.js グラフ、CSV/PDF出力
...
### 人員別按分方式
複数人運搬時は「記録の積込・運搬数量 × 対象人員の人工 ÷ 記録の合計人工」で按分。按分合計＝全体合計となることを確認済。
...
### 保存処理の一貫性 (POST / PUT)
- **POST（新規登録）**: 積込・運搬記録本体を単発で INSERT し `last_row_id` を得た後、積込・運搬人員を `db.batch()` で登録する。**厳密な単一トランザクションではない**。人員登録が失敗した場合は本体を **補償 DELETE** することで、部分的なゴミレコードが残らないようにする（compensating transaction）。補償 DELETE 自体が失敗した場合は HTTP 500 に `recordId` と両方のエラー詳細を含めて返却し、管理者が整合性を確認できるようにする。
...
- **プレビューのみ（本番未デプロイ／GitHub未push）**: **積込・運搬数量機能一式**（本 turn 追加、ユーザー承認待ち）
- **最終更新**: 2026-07-18 積込・運搬数量機能追加（ローカルのみ）
